...
export const refreshTokenUseCase = async (
    ...
    const existingToken = await refreshDbRepository.findRefreshToken(refreshToken);

    if (!existingToken) {
        throw new AppError('Refresh token not exists', HttpStatusCodes.UNAUTHORIZED);
    }
    const expirationDate = new Date(existingToken.expiresAt);
    if (new Date() > expirationDate) {
        throw new AppError('Refresh token has expired', HttpStatusCodes.UNAUTHORIZED);
    }
